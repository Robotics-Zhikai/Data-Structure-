...
public:
	...
	//重载operator以便能用到标准库 通过设置CompType来确定比较大小的维度
...
private:
	...
	{
		if (recorddist.size() < k) //如果还没满，则一直加入堆
		{
		...
		{
			if (disttmp < recorddist[0])//如果堆满了之后再进入新的数据，则判断该数据是否小于堆顶数据
			{							//如果小于则把堆顶数据弹出，把新数据推入。否则忽略该数据
	...
	{
		static int flagDownTree = 1; //设置为1时主要功能是下沉，为0时主要功能是回溯
		if (CurrentIndex == -1)
			return;
		input.CompType = depth%input.readDimension();

		if (input <= KdTreeDataflow[CurrentIndex]) //测试节点在当前节点的维度上小于当前节点 ，则先搜索左子树，在搜索右子树
		{
			...
			if (flagDownTree == 0)
			{
				...
				input.CompType = depth%input.readDimension();
				double distabs = abs(input.Data[input.CompType] - KdTreeDataflow[CurrentIndex].Data[input.CompType]);//计算与超平面的距离
				if (distabs > recorddist[0].value) 
					//说明堆中的最大值小于超平面距离，到另一分支的所有节点距离都大于现在找到的最大值，另一分支一定没有更小的，进行剪枝
					return;
				else
					flagDownTree = 1; //说明堆中的最大值大于等于超平面距离，另一分支不能剪枝，需要进入另一分支寻找可能的更近的值
		...
		flagDownTree = 0; //到叶子节点了，该执行回溯步骤了
		//到这里就到了叶子节点了
...
int DecideCategory(const vector<node>& NearkNodes)
{
	...
	{
		int current = cats[0];
		vector<int> countCat; //计算对应的种类的分布个数
		vector<int> UniqueCat; //将重复的k近邻种类数去重，只保留一个
...
vector<double> k_nearest_neighbors_KdTree(const KdTree &trainDataKdtree,vector<node> traindata, node testnode, unsigned int k)
{
	...
	auto searchresult = trainDataKdtree.Search_NearkIndex(k, testnode);//找到k近邻，核心是调用SearchNode
	...
	cout << "最终的KNN判定类别是：" << DecideCategory(NearkNode) << endl; //通过k个节点判断
...
void k_nearest_neighbors(vector<node> traindata, vector<node> testdata,unsigned int k)
{
	...
	for (size_t i = 0; i < testdata.size(); i++)
	{
		...
		endKdTree = clock();
		KdTreeT = (double)(endKdTree - startKdTree) / CLOCKS_PER_SEC; //记录KdTree方法实现的KNN的时间
		...
		Violence_searchT = (double)(endViolence_search - startViolence_search) / CLOCKS_PER_SEC;  //记录暴力搜索方法实现的KNN的时间

		vector<double> minus;
		for (size_t j = 0; j < kddist.size(); j++)
			minus.push_back(abs(kddist[j] - violencedist[j]));
		if (*max_element(minus.begin(), minus.end()) > 0) //用以验证KdTree实现的KNN是否出错
...
int main()
{
	try
	{
		...
		//3000000组数据 KDtree有明显速度提升 是暴力搜索效率的两倍
		//
		vector<node> traindata;
		addvector<node>(traindata, Catgery1data); //将三种类别的数据合并在一起
